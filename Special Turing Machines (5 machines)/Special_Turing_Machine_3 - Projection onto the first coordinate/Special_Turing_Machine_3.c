...
void podaci() {
	...
	printf("Test primer [3/5]: Funkcija projekcije na prvu koordinatu\n");
	printf("Mentor: dr Branko Malesevic, vanredni profesor\n");
	printf("Student: Nemanja Karapetrovic [broj indeksa: ****/****]\n");
	printf("Napomena: Potrebno je da se ulazna datoteka zove ulaz3.txt i da se nalazi gde i izvorni fajl Special_Turing_Machine_3.c .\n");
...
Turing* obrada(Turing* lst) {
	Turing* glava;
	glava = lst;
	while (glava->pod == ' ') glava = glava->sled; // Petlja pozicionira glavu na krajnji levi simbol koji nije blanko znak.
	int q1 = 0; /* Indikator: q1=1, program je bio u q1 stanju
							  q1=0, program nije bio u q1 stanju  */


							  /* Stranje q0 => f(q0,0)=(q-,0,+1);
											   f(q0,1)=(q1,1,+1);   */
...
void ispis(Turing* lst) {
	...
	printf("f(q1,b)=(q-,b,+1)\n");
	printf("f(q1,0)=(q2,b,+1)\n");
	printf("f(q1,1)=(q1,1,+1)\n");
	printf("f(q2,b)=(q+,b,-1)\n");
	printf("f(q2,0)=(q-,0,+1)\n");
	printf("f(q2,1)=(q2,b,+1)\n");
	puts(" ");
	...
	puts(" ");
	if (izbor1 == '1') printf("Vasa datoteka je uspesno formirana. Njen naziv je izlaz3.txt. Nalazi se na adresi izvornog fajla TuringMachine3.c .\n");
}
